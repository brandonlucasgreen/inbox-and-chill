...
- **Priority Inbox is NOT reachable from a personal API key — verified live 2026-09-14**, against the real endpoint with a real key. Linear shipped the feature on 2026-09-03; the SDK schema was regenerated on 2026-09-09 and carries no trace of it on a notification. All three doors are shut, and each was checked rather than reasoned about: live introspection of `Notification` returns 28 fields with **no `isPriority`** (querying it is a validation error, not a null); `NotificationFilter` accepts only `id`, `createdAt`, `updatedAt`, `type`, `subscriptionType`, `archivedAt`, `and`, `or`, so there is **no server-side priority filter either**; and the readable `UserSettings` type carries neither `priorityInboxEnabled` nor the Priority filter config — both exist only on the **write-only** `UserSettingsUpdateInput`, so even the user's own configuration cannot be read back. Linear's first-party MCP server *does* return `isPriority` per notification, so the flag is real and Linear simply does not expose it publicly; treat that as a measuring stick, never as a data source the app can call.
  - **What we use instead: `category`** (`NotificationCategory`, 17 values, public and not `[Internal]`) — one checkbox per value on the Linear source, all on by default. It is Linear's own vocabulary, so the UI invents no concepts, and it must stay a **drop-list**: an unrecognised category is kept, or the first value Linear adds vanishes from the queue silently.
  - Measured against the MCP's flag over 50 real notifications: Linear's Priority tab excludes only **11 of 50** — 7 `subscriptions`, 2 `postsAndUpdates` (both `initiativeUpdateCreated`), 1 `feed`, 1 `reactions`. Unticking `subscriptions`, `reactions` and `feed` reproduces its verdict on **48 of 50** with no false drops. The connector's own `isHighSignal` set is *not* the right basis: it agrees on only 23 of 50 and hides 27 that Linear calls priority.
  - **If OAuth or a Linear app is ever revisited, re-check this first** — the argument to beat is a field that does not exist for personal keys, not a missing scope.
