...
| **Buffer** | ⏸ Wanted, **API not there yet** (re-checked 2026-09-14) | The 2026-09-09 "Engagement API" is a per-channel *capability* table (`configuration`) — no query returns a comment or mention, no reply mutation, no webhook. Comment-count deltas on `Post.metrics` are buildable now as a "go look" row. Rate limit (250 req/day) forces a ≥15-min poll. `docs/buffer-engagement-plan.md`. |
